...
export const WithdrawModal: React.FC<WithdrawModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      const fetchSOLPrice = async () => {
        try {
          const response = await fetch('/api/oracle/price/SOL');
          if (response.ok) {
            const data = await response.json();
            const p = Number(data?.price ?? data?.value);
            if (Number.isFinite(p) && p > 0) {
              setSolPrice(p);
              console.log('💰 Using real-time SOL price:', p);
            }
          }
        } catch (error) {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const signature = await SmartContractService.getInstance().withdrawNativeSOL(wallet, amountInLamports);
