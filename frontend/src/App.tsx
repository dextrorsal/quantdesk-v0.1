...
function AppRoutes() {
  ...
  // Explicit adapters per APP.md so brands appear by default, deduped and filtered
  const walletCandidates = React.useMemo(() => [
  ...
  const wallets = React.useMemo(() => {
    ...
    for (const w of walletCandidates) {
      if (w.readyState !== WalletReadyState.Unsupported && !map.has(w.name)) {
        map.set(w.name, w)
      }
    }
  ...
  const DevProgramWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    if (!anchorWallet) {
      return <div className="p-6 text-white" style={{ pointerEvents: 'none' }}>Connect your wallet to load the program…</div>;
    }
    return (
            ...
            <MarketProvider>
              <PriceProvider websocketUrl="ws://localhost:3002/" fallbackApiUrl="/api/prices">
                <TradingProvider>
                  ...
                  <Route path="/devnet-testing" element={
                    <Layout>
                      <DevProgramWrapper>
                        <DevnetTestingPage />
                      </DevProgramWrapper>
