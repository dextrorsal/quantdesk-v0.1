...
// Your program ID
const PROGRAM_ID = new PublicKey("GcpEyGMJg9AvEx8LgAyzyUNTPWFxovHxtmQ4vVWuWu3a");
...
export const ProgramProvider: React.FC<ProgramProviderProps> = ({
    ...
    const createUserAccount = useCallback(async (
        ...
        try {
            // Derive user account PDA using helper
    ...
    // Memoize user account PDA (don't recompute on every render)
    const userAccountPDA = useMemo(() => {
        if (!program || !wallet.publicKey) return null;
        return deriveUserAccountPDA(program, wallet.publicKey, 0)[0];
