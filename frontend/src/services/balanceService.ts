...
class BalanceService {
  ...
  async getUserBalances(walletAddress: PublicKey): Promise<UserBalances> {
    try {
      ...
      // Get BOTH actual wallet SOL balance AND collateral balance
      const walletSOLBalance = await this.getNativeSOLBalance(walletAddress);
      ...
      // Include both wallet balance and collateral balance
      let totalValueUSD = (walletSOLBalance + collateralBalance) * solPrice;
      ...
      const userBalances: UserBalances = {
        nativeSOL: walletSOLBalance + collateralBalance, // Both wallet SOL and deposited collateral
        tokens,
        totalValueUSD,
      };
      
      console.log('💰 Wallet SOL balance:', walletSOLBalance, 'SOL');
      console.log('💰 Collateral balance:', collateralBalance, 'SOL');
      console.log('💰 Total SOL:', walletSOLBalance + collateralBalance, 'SOL');
