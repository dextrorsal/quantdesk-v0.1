...
class PortfolioService {
  ...
  private generateMockData(): void {
    ...
    symbols.forEach((symbol, index) => {
      ...
      const entryPrice = this.getRealPrice(symbol)
  ...
  private getRealPrice(symbol: string): number {
    const priceData = priceStore.getPrice(symbol)
    if (priceData && priceData.price > 0) {
      return priceData.price
    }
    
    // Fallback to mock prices if real price not available
  ...
  private generateMockTrades(): void {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const price = this.getRealPrice(symbol) + (Math.random() - 0.5) * (this.getRealPrice(symbol) * 0.02)
