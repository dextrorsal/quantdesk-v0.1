...
export class WebSocketService {
  ...
  private isOptional = true // Make WebSocket optional by default
  private connectionFailed = false // Track if connection has failed

  constructor(url: string = 'ws://localhost:3002/', optional: boolean = true) {
    this.url = url
  ...
  public connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      // If connection already failed and WebSocket is optional, don't try again
      if (this.connectionFailed && this.isOptional) {
        console.log('🔌 WebSocket connection previously failed, skipping (optional mode)')
      ...
      try {
        ...
        this.ws.onclose = (event) => {
          ...
          // Only attempt reconnection if not optional or if we haven't failed before
          if (!this.isOptional || !this.connectionFailed) {
        ...
        this.ws.onerror = (error) => {
          if (this.isOptional) {
            // console.log('⚠️ WebSocket connection failed (optional mode)')
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      setTimeout(() => {
        this.connect().catch((error) => {
          if (this.isOptional) {
            // console.log('⚠️ WebSocket reconnection failed (optional mode)')
            this.connectionFailed = true
          } else {
      ...
      if (this.isOptional) {
        // console.log('⚠️ Max reconnection attempts reached (optional mode)')
        this.connectionFailed = true
      } else {
  ...
  public getConnectionInfo(): { connected: boolean; available: boolean; failed: boolean } {
    return {
      connected: this.isConnected,
      available: !this.connectionFailed,
