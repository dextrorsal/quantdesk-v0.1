...
export const useWalletAuth = () => {
  ...
  useEffect(() => {
    const checkAuthStatus = async () => {
      if (connected && publicKey) {
        const user = await walletAuthService.instance.checkSession();
        if (user) {
  ...
  const authenticate = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const nonceResponse = await walletAuthService.instance.getNonce(walletPubkey);
      ...
      const authMessage = walletAuthService.instance.createAuthMessage(walletPubkey, nonce);

      // 3. Sign the message with the wallet
      const signature = await signMessage(new TextEncoder().encode(authMessage.message));

      // 4. Verify signature with backend
      const response = await walletAuthService.instance.verifySignature(
        walletPubkey,
        signature,
        nonce,
      );
      ...
      if (response.success) {
        ...
        const userProfile = await walletAuthService.instance.fetchUserProfile();
        setState(prev => ({
          ...prev,
  ...
  const logout = useCallback(async () => {
    await walletAuthService.instance.logout();
