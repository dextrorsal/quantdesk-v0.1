...
# Find and kill any Node processes running on port 3000 (alternative frontend port)
PORT_3000_PID=$(lsof -t -i:3000)
if [ -n "$PORT_3000_PID" ]; then
  echo -e "${YELLOW}Found process on port 3000 with PID: $PORT_3000_PID. Killing...${NC}"
  kill -9 "$PORT_3000_PID"
  echo -e "${GREEN}✅ Process on port 3000 killed.${NC}"
else
