...
class Config:
        ...
        if config_file_path.exists():
            try:
                with open(config_file_path, "r") as f:
                    ...
                    print(
                        f"Debug: JSON config loaded successfully from: "
                        f"{config_path}"
                    )
            ...
            print(
                f"Warning: {config_path} not found, using default settings."
            )
        ...
        try:
            for key in keys:
                ...
                if not isinstance(key, str):
                    raise TypeError(
                        f"Each key must be a string, but got {key!r} of type "
                        f"{type(key).__name__}."
                    )
                ...
                if isinstance(current_level, dict) and key in current_level:
                    ...
                    print(
                        f"DEBUG: Requested {keys} → now at level: {masked_display}"
                    )
                else:
            ...
            raise TypeError(
                ...
                f"Ensure all intermediate values are dicts. Current level: "
                f"{current_level}"
            ) from e
    ...
    def _init_storage_config(self) -> StorageConfig:
        ...
        try:
            ...
            raise ConfigurationError(
                f"Failed to initialize storage config: {e}"
            )
    ...
    def _init_exchange_configs(self) -> Dict[str, ExchangeConfig]:
        ...
        # Bitget Mainnet
        if self.get("BITGET_ENABLED", default="true").lower() == "true":
            key_path = self.get("BITGET_KEY_PATH")
            ...
            if key_path:
                try:
                    ...
                    print(f"Warning: Could not load Bitget mainnet keys from {key_path}: {e}")
            configs["bitget"] = ExchangeConfig(
                name="bitget",
                credentials=ExchangeCredentials(
                    api_key=api_key,
                    api_secret=api_secret,
                    additional_params={"passphrase": passphrase} if passphrase else {},
                ),
                rate_limit=int(
                    self.get("BITGET_RATE_LIMIT", default="20")
                ),
                markets=self.get(
                    "BITGET_MARKETS", default="BTCUSDT"
                ).split(","),
                base_url=self.get(
        ...
        # Bitget Demo
        if self.get("BITGET_DEMO_ENABLED", default="true").lower() == "true":
            configs["bitget_demo"] = ExchangeConfig(
                name="bitget_demo",
                credentials=ExchangeCredentials(
                    api_key=self.get("BITGET_DEMO_KEY"),
                    api_secret=self.get("BITGET_DEMO_SK"),
                ),
                rate_limit=int(
                    self.get("BITGET_DEMO_RATE_LIMIT", default="20")
                ),
                markets=self.get(
                    "BITGET_DEMO_MARKETS", default="BTCUSDT"
                ).split(","),
                base_url=self.get(
                    "BITGET_DEMO_BASE_URL", default="https://api.bitget.com"
                ),
                enabled=True,
            )
