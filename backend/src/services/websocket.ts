...
export class WebSocketService {
  ...
  private async authenticateUser(token: string): Promise<any> {
    try {
      ...
      const user = await this.db.getUserByWalletAddress(decoded.walletAddress);
      return user;
    } catch (error) {
  ...
  private startDataBroadcasting(): void {
    ...
    // Broadcast order book updates every 1 second
    setInterval(async () => {
      try {
  ...
  private async getMarketData(symbol: string): Promise<MarketDataUpdate | null> {
    try {
      ...
      // Get 24h stats (simplified - in production, calculate from trades)
      ...
      const volume24h = parseFloat(row.volume_usd) || 0;
      const tradesCount = parseInt(row.trades_count) || 0;

      // Calculate 24h change (simplified)
      const change24h = 0; // In production, calculate from historical prices

      // Get open interest
      const openInterest = await this.db.query(
