...
class PythOracleService {
  ...
  public async fetchLatestPrices(): Promise<Map<string, PythPriceData>> {
    ...
    try {
      ...
      if (priceMap.size > 0) {
        ...
        this.logger.error('❌ Pyth API returned no prices - this is a critical failure')
        throw new Error('Pyth Network returned no price data')
      }
    } catch (error) {
      this.logger.error(`❌ Error fetching latest prices from Pyth: ${error.message}`, error)
      throw error // Don't fall back to CoinGecko - let caller handle fallback
    }
  }
  ...
  public async getLatestPrice(marketSymbol: string): Promise<number | null> {
    const endTimer = performanceMonitoringService.startTimer('oracle_call') as (success?: boolean, error?: string, userId?: string) => void;
    ...
    try {
      ...
      if (!symbol) {
        endTimer(false, 'Invalid market symbol');
        return null;
      }
