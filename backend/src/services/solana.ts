...
export class SolanaService {
  ...
  private constructor() {
    ...
    this.programId = new PublicKey(config.QUANTDESK_PROGRAM_ID);
  }
  ...
  public async getAccountInfo(address: string): Promise<AccountInfo<Buffer> | null> {
    try {
      ...
      const accountInfo = await this.rpcLoadBalancer.getAccountInfo(publicKey);
      return accountInfo;
    } catch (error) {
  ...
  public async getMultipleAccountsInfo(addresses: string[]): Promise<(AccountInfo<Buffer> | null)[]> {
    try {
      ...
      const accountsInfo = await this.rpcLoadBalancer.executeWithRetry(async (connection) => {
  ...
  public async getProgramAccounts(filters?: any[]): Promise<SolanaAccount[]> {
    try {
      const accounts = await this.rpcLoadBalancer.executeWithRetry(async (connection) => {
        return await connection.getProgramAccounts(this.programId, {
          filters: filters || []
        });
      });
  ...
  private async syncPositions(): Promise<void> {
    try {
      ...
      const users = await this.db.select('users', 'wallet_address');
  ...
  public async getTransactionHistory(address: string, limit: number = 10): Promise<any[]> {
    try {
      const publicKey = new PublicKey(address);
      const signatures = await this.rpcLoadBalancer.executeWithRetry(async (connection) => {
        return await connection.getSignaturesForAddress(publicKey, { limit });
      });
      
      const transactions = [];
      for (const sig of signatures) {
        const tx = await this.rpcLoadBalancer.executeWithRetry(async (connection) => {
  ...
  public async getCurrentSlot(): Promise<number> {
    try {
      return await this.rpcLoadBalancer.executeWithRetry(async (connection) => {
   ...
   */
  public getRPCStats(): any {
    return this.rpcLoadBalancer.getProviderStats();
  }
  ...
  public getConnection(): Connection {
    return this.rpcLoadBalancer.getConnection();
  }
}
