...
import mikeyProxyRoutes from './routes/mikeyProxy';
...
// Import Redis client
import { connectRedis, getRedisSubscriber, pingRedis } from './services/redisClient';
...
import { conditionalOrderMonitor } from './services/conditionalOrderMonitor';
import { redisCacheService } from './services/redisCache';
import { Logger } from './utils/logger';
import { getSystemMonitor } from './services/systemMonitor';
import { config, validateConfig } from './config/environment';
// import { PythReceiverWorker, getEnvFeedIds } from './services/pythReceiverWorker'; // Disabled - on-chain Pyth Receiver not needed for now

// Load environment variables
// 1) Load backend/.env (if present)
dotenv.config();
// 2) Also load project-root/.env so the backend sees the root env file
try {
...
app.get('/health', async (_req, res) => {
  // Check Redis health
  const redisHealth = await redisCacheService.getHealthStatus();
...
app.get('/api/prices', async (req, res) => {
  try {
    ...
    let lastIngestAt = 0;
    const STALE_MS = Number(process.env['PRICE_STALE_MS'] || 3000);
    const MAX_CONF_RATIO = Number(process.env['PRICE_MAX_CONF_RATIO'] || 0.01); // 1%
    for (const [asset, p] of prices.entries()) {
      if (!p) continue;
      const updatedAt = typeof p.timestamp === 'number' ? p.timestamp : Date.now();
      const publishTime = typeof p.publishTime === 'number' ? p.publishTime : updatedAt;
      if (updatedAt > lastIngestAt) lastIngestAt = updatedAt;
      const price = p.price;
      const confidence = p.confidence ?? null;
      const ema = p.ema ?? null;
      const feedId = p.feedId ?? null;

      // classify status
      let status: 'live'|'stale'|'high_conf'|'fallback'|'unavailable' = 'live';
      let reason: string | undefined;
      if (typeof price !== 'number' || !isFinite(price)) {
        status = 'unavailable';
        reason = 'invalid_price';
      } else if (Date.now() - updatedAt > STALE_MS) {
        status = 'stale';
        reason = `stale>${STALE_MS}ms`;
      } else if (typeof confidence === 'number' && confidence > 0 && price > 0 && (confidence / price) > MAX_CONF_RATIO) {
        status = 'high_conf';
        reason = `conf_ratio>${MAX_CONF_RATIO}`;
      }
      ...
      data[asset] = {
        ...
        publishTime,
        feedId,
        // 24h stats can be augmented by a secondary later
    ...
    res.json({ success: true, data, lastIngestAt, timestamp: Date.now() });
  } catch (error) {
...
// Canary/health for price hub
app.get('/api/prices/health', async (_req, res) => {
  try {
    const prices = await pythOracleService.fetchLatestPrices();
    let lastIngestAt = 0;
    let assetCount = 0;
    for (const [, p] of prices.entries()) {
      if (!p) continue;
      assetCount += 1;
      const ts = typeof p.timestamp === 'number' ? p.timestamp : 0;
      if (ts > lastIngestAt) lastIngestAt = ts;
    }
    res.json({ ok: assetCount > 0, lastIngestAt, assetCount });
  } catch (e) {
    res.status(503).json({ ok: false, error: 'unavailable' });
  }
});

// --- Price Hub helpers and extended endpoints ---
...
function normalizeAssetSymbol(input: string): string {
  ...
  if (m && m[1]) return m[1];
  return upper.replace(/[^A-Z]/g, '').replace(/(USDT|USD|PERP)$/,'');
}

// Single-asset endpoint aligned with map shape
app.get('/api/price/:asset', async (req, res) => {
  try {
    const asset = normalizeAssetSymbol(req.params.asset);
    if (!asset) return res.status(400).json({ success: false, error: 'invalid_asset' });
    desiredAssets.add(asset);
    const prices = await pythOracleService.fetchLatestPrices();
    const p = prices.get(asset);
    if (!p) return res.status(200).json({ success: true, data: { status: 'unavailable', source: 'pyth' } });
    const STALE_MS = Number(process.env['PRICE_STALE_MS'] || 3000);
    const MAX_CONF_RATIO = Number(process.env['PRICE_MAX_CONF_RATIO'] || 0.01);
    const updatedAt = typeof p.timestamp === 'number' ? p.timestamp : Date.now();
    const publishTime = typeof p.publishTime === 'number' ? p.publishTime : updatedAt;
    const confidence = p.confidence ?? null;
    const price = p.price;
    let status: 'live' | 'stale' | 'high_conf' = 'live';
    let reason: string | undefined;
    if (typeof price !== 'number' || !isFinite(price)) { status = 'stale'; reason = 'invalid_price'; }
    else if (Date.now() - updatedAt > STALE_MS) { status = 'stale'; reason = `stale>${STALE_MS}ms`; }
    else if (typeof confidence === 'number' && confidence > 0 && price > 0 && (confidence / price) > MAX_CONF_RATIO) { status = 'high_conf'; reason = `conf_ratio>${MAX_CONF_RATIO}`; }
    res.json({ success: true, data: { price, confidence, ema: p.ema ?? null, updatedAt, publishTime, feedId: p.feedId ?? null, change24h: p.change24h ?? null, volume24h: p.volume24h ?? null, high24h: p.high24h ?? null, low24h: p.low24h ?? null, source: 'pyth', status, ...(reason ? { reason } : {}) } });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err?.message || 'price_error' });
  }
});

// Subscribe desired assets (best-effort; underlying service may be pull-based)
app.post('/api/prices/subscribe', async (req, res) => {
  try {
    const body = req.body || {};
    const assets: string[] = Array.isArray(body.assets) ? body.assets : [];
    const normalized = assets.map(normalizeAssetSymbol).filter(Boolean);
    for (const a of normalized) desiredAssets.add(a);
    if (typeof (pythOracleService as any).subscribeAssets === 'function') {
      try { await (pythOracleService as any).subscribeAssets(Array.from(desiredAssets)); } catch {}
    }
    res.json({ success: true, subscribed: Array.from(desiredAssets) });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err?.message || 'subscribe_error' });
  }
});
...
app.use('/api/twitter', twitterRoutes); // Twitter feed routes
app.use('/api/news', newsRoutes); // News aggregation routes
app.use('/api/mikey', mikeyProxyRoutes); // Proxy to MIKEY-AI (avoids CORS)
app.use('/api/redis', redisRoutes); // Redis health, stats, cache management
...
server.listen(PORT, () => {
  ...
  logger.info(`🔍 Starting conditional order monitor service...`);
  conditionalOrderMonitor.setWebSocketService(wsService);
  conditionalOrderMonitor.start();
  logger.info(`✅ Conditional order monitor service started`);

  // Start Pyth Receiver pull-integration worker (dry-mode without wallet) - DISABLED for now
  // Using Hermes off-chain instead; on-chain receiver can be enabled later if needed
  /*
  try {
    const FEED_IDS = getEnvFeedIds();
    if (FEED_IDS.length > 0) {
      const worker = new PythReceiverWorker({
        rpcUrl: process.env['VITE_SOLANA_RPC_URL'] || process.env['SOLANA_RPC_URL'] || 'https://api.devnet.solana.com',
        receiverProgramId: process.env['PYTH_SOLANA_RECEIVER_PROGRAM_ID'] || 'PiThtEceivEr111111111111111111111111111111',
        feedIds: FEED_IDS,
        walletSecretBase64: process.env['PYTH_RECEIVER_WALLET_B64'] || undefined,
        batchSize: Number(process.env['PYTH_RECEIVER_BATCH'] || 10),
        intervalMs: Number(process.env['PYTH_RECEIVER_INTERVAL_MS'] || 5000),
      });
      worker.start();
      logger.info(`🛰️ Pyth Receiver worker started for ${FEED_IDS.length} feeds`);
    } else {
      logger.warn('No hex feed IDs found in env (PYTH_PRICE_FEED_*). Receiver worker not started.');
    }
  } catch (e: any) {
    logger.warn('Failed to start Pyth Receiver worker:', e?.message || e);
  }
  */
