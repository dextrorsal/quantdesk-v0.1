...
router.get('/:symbol/price', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const priceResult = await mcpSupabaseService.executeSQL(`
      SELECT 
        op.price,
        op.confidence,
        op.created_at
...
router.get('/:symbol/price-history', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const historyResult = await mcpSupabaseService.executeSQL(`
      SELECT 
        op.price,
        op.created_at
      FROM markets m
...
router.get('/:symbol/funding', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const fundingResult = await mcpSupabaseService.executeSQL(`
      SELECT 
