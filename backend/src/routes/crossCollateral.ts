...
router.post('/swap', async (req: Request, res: Response) => {
  try {
    ...
    if (!Object.values(CollateralType).includes(swapRequest.from_asset as CollateralType) || 
        !Object.values(CollateralType).includes(swapRequest.to_asset as CollateralType)) {
      return res.status(400).json({
        success: false,
        error: 'Invalid asset type. Supported types: SOL, USDC, BTC, ETH, USDT'
      });
...
router.get('/max-borrowable/:userId', async (req: Request, res: Response) => {
  try {
    ...
    const maxBorrowable = await crossCollateralService.calculateMaxBorrowableAmount(userId, collateralType as string || 'USDC');
...
router.post('/update-values', async (req: Request, res: Response) => {
  try {
    ...
    await crossCollateralService.updateCollateralValues(userId || 'default-user');
