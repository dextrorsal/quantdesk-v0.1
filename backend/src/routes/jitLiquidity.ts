...
router.get('/auctions', async (req: Request, res: Response) => {
  try {
    const auctions = await jitLiquidityService.getActiveAuctions();
...
router.post('/auctions', async (req: Request, res: Response) => {
  try {
    ...
    const auction = await jitLiquidityService.createAuction({
      marketId,
      side,
      size,
      price,
...
router.post('/auctions/:auctionId/bid', async (req: Request, res: Response) => {
  try {
    ...
    const success = await jitLiquidityService.submitBid(
      auctionId,
      {
        userId,
        bidPrice,
...
router.get('/market-makers', async (req: Request, res: Response) => {
  try {
    const marketMakers = await jitLiquidityService.getAllMarketMakers();
...
router.get('/liquidity-mining', async (req: Request, res: Response) => {
  try {
    const programs = await jitLiquidityService.getAllLiquidityMiningPrograms();
...
router.get('/price-improvements', async (req: Request, res: Response) => {
  try {
    ...
    const marketId = req.query.marketId as string || 'SOL-PERP';
    const timeframe = req.query.timeframe as string || '1h';
    const priceImprovements = await jitLiquidityService.getPriceImprovements(marketId, timeframe);
...
router.get('/strategies', async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    const strategies = await jitLiquidityService.getMarketMakingStrategies();
...
router.post('/strategies', async (req: Request, res: Response) => {
  try {
    ...
    const strategy = await jitLiquidityService.createMarketMakingStrategy({
      userId,
      marketId,
      strategyType,
      parameters
