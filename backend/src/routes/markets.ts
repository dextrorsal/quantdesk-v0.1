  ...
  try {
    ...
    const marketData = await Promise.all(
      markets.map(async (market) => {
        ...
        // Calculate open interest from positions
        ...
        const openInterest = activePositions.reduce((sum, pos) => sum + (pos.size * pos.entry_price), 0);
        
        // Calculate 24h price change from oracle_prices table
        const priceHistory = await oracle.getPriceHistory(market.symbol, 24);
        let priceChange24h = 0;
        if (priceHistory.length >= 2) {
          const currentPrice = oraclePrice?.price || 0;
          const yesterdayPrice = priceHistory[0]?.price || 0;
          if (yesterdayPrice > 0) {
