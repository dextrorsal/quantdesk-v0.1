...
// Conditional Order Monitor metrics
router.get('/conditional-orders', async (_req: Request, res: Response) => {
  try {
    const data = conditionalOrderMonitor.getMetrics();
...
router.get('/conditional-orders/state', async (_req: Request, res: Response) => {
  try {
    const data = conditionalOrderMonitor.getState();
    res.json({ success: true, data, timestamp: Date.now() });
  } catch (error) {
    res.status(500).json({ success: false, error: (error as Error).message });
  }
