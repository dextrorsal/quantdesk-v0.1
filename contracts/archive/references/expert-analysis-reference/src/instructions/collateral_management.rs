...
pub fn withdraw_native_sol(ctx: Context<WithdrawNativeSol>, amount: u64) -> Result<()> {
    ...
    require!(user_account.available_margin >= amount, ErrorCode::InsufficientCollateral);
    ...
    // Update user account total collateral
    user_account.total_collateral -= amount;
    user_account.available_margin -= amount;
    
    msg!("Withdrew {} SOL from collateral account", amount);
    Ok(())
}
