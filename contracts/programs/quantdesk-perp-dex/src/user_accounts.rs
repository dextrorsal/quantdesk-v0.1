...
// This handles the initial account creation and management for users
...
impl UserAccount {
    ...
    //      +2 (liquidation_threshold) +2 (max_leverage) +8 (funding_paid) +8 (funding_received) 
    //      +8 (fees_paid) +8 (rebates_earned) = +70 bytes total
}
...
impl UserAccount {
    ...
    pub fn remove_collateral(&mut self, amount: u64) -> Result<()> {
        require!(self.total_collateral >= amount, UserAccountError::InsufficientCollateral);
        self.total_collateral -= amount;
        self.update_activity()?;
        Ok(())
    }
    ...
    pub fn remove_position(&mut self) -> Result<()> {
        require!(self.total_positions > 0, UserAccountError::NoPositionsToRemove);
        self.total_positions -= 1;
        self.update_activity()?;
        Ok(())
    }
    ...
    pub fn remove_order(&mut self) -> Result<()> {
        require!(self.total_orders > 0, UserAccountError::NoOrdersToRemove);
        self.total_orders -= 1;
        self.update_activity()?;
        Ok(())
    }
    ...
    pub fn update_account_health(&mut self, health: u16) -> Result<()> {
        require!(health <= 10000, UserAccountError::InvalidHealthValue);
    ...
    pub fn deactivate(&mut self) -> Result<()> {
        require!(self.total_positions == 0, UserAccountError::AccountHasPositions);
        require!(self.total_orders == 0, UserAccountError::AccountHasOrders);
    ...
    // NEW: Margin calculation methods (Drift-style)
...
// Additional error codes for user account management
#[error_code]
