...
/// Place an advanced order with enhanced validation and idempotency
pub fn place_order(
    ...
    twap_duration: u64,
    twap_interval: u64,
    idempotency_key: [u8; 32], // Add idempotency key parameter
) -> Result<()> {
    ...
    let existing_order = &ctx.accounts.order;
    if existing_order.idempotency_key == idempotency_key && existing_order.status != OrderStatus::Cancelled {
        msg!("⚠️ Duplicate order detected with idempotency key");
        return Err(ErrorCode::DuplicateOrder.into());
    }

    // Initialize order
    let order = &mut ctx.accounts.order;
    order.user = ctx.accounts.user.key();
    order.idempotency_key = idempotency_key; // Set idempotency key
