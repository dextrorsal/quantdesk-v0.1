...
pub struct KeeperSecurityManager {
    ...
    // Gas optimization: Use fixed-size array for authorized keepers
    pub authorized_keepers: [KeeperAuth; 3], // Reduced from 5 to 3 for maximum stack efficiency
    ...
    // Audit trail (gas-optimized with fixed-size array)
    pub liquidation_history: [LiquidationRecord; 5], // Reduced from 10 to 5 for maximum stack efficiency
    pub history_index: u8,                    // Current index in circular buffer
...
pub struct KeeperAuth {
    ...
    pub is_active: u8, // Changed from bool to u8 for ZeroCopy compatibility (0=false, 1=true)
...
pub struct LiquidationRecord {
    ...
    pub timestamp: i64,
    pub success: u8, // Changed from bool to u8 for ZeroCopy compatibility (0=false, 1=true)
...
impl KeeperSecurityManager {
    ...
    pub fn authorize_keeper(&mut self, keeper_pubkey: Pubkey, stake_amount: u64, 
        ...
        require!(self.keeper_count < 3, ErrorCode::TooManyKeepers);
    ...
    pub fn is_keeper_authorized(&self, keeper_pubkey: &Pubkey) -> Result<bool> {
        ...
        let is_authorized = keeper.is_active == 1 &&
    ...
    pub fn record_liquidation(&mut self, keeper_pubkey: Pubkey, position_owner: Pubkey,
        ...
        // Add to history
        let _record = LiquidationRecord {
            keeper_pubkey,
            position_owner,
            position_size,
            liquidation_price,
            timestamp: current_time,
            success: if success { 1 } else { 0 }, // Convert bool to u8
            reason,
        };
...
impl OracleStalenessProtection {
    ...
    pub fn get_best_price(&mut self, primary_price: u64, primary_timestamp: i64,
        ...
        if let (Some(sec_price), Some(sec_timestamp)) = (secondary_price, secondary_timestamp) {
            let _secondary_staleness = current_time - sec_timestamp;
