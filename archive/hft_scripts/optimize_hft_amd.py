#!/usr/bin/env python3
"""
🚀 AMD ROCm HFT Model Optimization

Optimizes HFT models using AMD ROCm GPU acceleration.
Specifically configured for AMD Radeon RX 6750 XT.
...
class AMDHFTOptimizer:
    """AMD ROCm-optimized HFT optimizer."""
    ...
    def __init__(self, data_dir: str = "data/hft_consolidated"):
        ...
        # Check AMD ROCm GPU availability
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.logger.info(f"Using device: {self.device}")
        
        if torch.cuda.is_available():
            self.logger.info(f"GPU: {torch.cuda.get_device_name(0)}")
            self.logger.info(f"ROCm PyTorch: {torch.__version__}")
        
        # Optimization parameters (optimized for AMD GPU)
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Prepare all features using AMD GPU acceleration."""
        self.logger.info("Preparing features with AMD GPU acceleration...")
        ...
        # Calculate features (GPU accelerated)
        ...
        self.logger.info(f"Prepared {len(features_df)} rows with AMD GPU-accelerated features")
        return features_df
    ...
    def cross_validate_model(self, df: pd.DataFrame, params: Dict, 
        ...
        """Perform time series cross-validation with AMD GPU acceleration."""
        ...
        for train_idx, test_idx in tscv.split(X):
            ...
            # Convert to tensors and move to AMD GPU
    ...
    def optimize_hyperparameters(self, df: pd.DataFrame, 
        ...
        self.logger.info("Starting AMD GPU-accelerated hyperparameter optimization...")
    ...
    def train_optimized_model(self, df: pd.DataFrame, optimization_result: Dict) -> Tuple[LorentzianANN, Dict]:
        """Train the optimized model on full dataset with AMD GPU."""
        self.logger.info("Training optimized model with AMD GPU acceleration...")
        ...
        # Fit model
        prices = torch.tensor(features_df['close'].values, dtype=torch.float32).to(self.device)
    ...
    def generate_optimization_report(self, optimization_result: Dict, 
        ...
        report_file = Path(f"results/hft_optimization_report_{symbol}_{timeframe}_amd.md")
        report_file.parent.mkdir(parents=True, exist_ok=True)
        
        with open(report_file, 'w') as f:
            f.write(f"# AMD ROCm HFT Model Optimization Report - {symbol} {timeframe}\n\n")
            f.write(f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            
            f.write("## AMD GPU Configuration\n\n")
        ...
        self.logger.info(f"AMD optimization report saved to {report_file}")

def main():
    parser = argparse.ArgumentParser(description="Optimize HFT models with AMD ROCm")
    ...
    model_path = f"models/hft_optimized/{args.exchange}/{args.symbol}/{args.timeframe}/lorentzian_optimized_amd.pt"
    os.makedirs(os.path.dirname(model_path), exist_ok=True)
    model.save_model(model_path)
    
    # Print summary
    print(f"\n🎯 AMD ROCm HFT Model Optimization Complete!")
    ...
    print(f"📄 Report: results/hft_optimization_report_{args.symbol}_{args.timeframe}_amd.md")
