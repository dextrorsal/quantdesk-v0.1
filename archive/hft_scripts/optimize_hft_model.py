#!/usr/bin/env python3
"""
🚀 HFT Model Optimization Script

Optimizes HFT models to achieve >60% win rate through:
...
class HFTOptimizer:
    """Optimizes HFT models for better performance."""
    ...
    def __init__(self, data_dir: str = "data/hft_consolidated"):
        ...
        # Optimization parameters - reduced set for faster execution
        self.optimization_params = {
            'lookback_periods': [15, 20],
            'prediction_bars': [4, 6],
            'k_neighbors': [15, 20],
            'rsi_periods': [14],
            'adx_periods': [14],
        ...
        # Feature combinations to test - reduced for faster execution
        self.feature_combinations = [
            ['rsi', 'adx', 'cci', 'wave_trend'],  # Core indicators
            ['rsi_buy', 'rsi_sell', 'adx_buy', 'adx_sell'], # Buy/sell signals
            ['price_change', 'volume_change', 'volatility_5'], # Price dynamics
            ['momentum_5', 'high_low_ratio', 'close_open_ratio'] # Momentum and ratios
        ]
    ...
    def prepare_features_with_params(self, df: pd.DataFrame, params: Dict) -> pd.DataFrame:
        """Prepare features with specific parameters."""
        features_df = df.copy()
        
        # RSI with custom period
        rsi = RSIIndicator(period=params.get('rsi_period', 14))
        ...
        adx = ADXIndicator(period=params.get('adx_period', 14))
        ...
        cci = CCIIndicator(period=params.get('cci_period', 14))
        ...
        # WaveTrend with custom parameters
        wt = WaveTrendIndicator(
            channel_length=params.get('wt_period', 14),
            average_length=params.get('wt_period', 14) + 1,
            smoothing_length=4
        )
        wt_features = wt.calculate_signals(df)
    ...
    def cross_validate_model(self, df: pd.DataFrame, params: Dict, 
        ...
        features_df = self.prepare_features_with_params(df, params)
    ...
    def optimize_hyperparameters(self, df: pd.DataFrame, 
        ...
        # Test feature combinations directly - more efficient for a focused optimization
        for feature_combo in [
            # Test individual feature groups
            [0], [1], [2], [3],
            # Test pairs of feature groups
            [0, 1], [0, 2], [0, 3], [1, 2], [1, 3], [2, 3],
            # Test combination of all groups
            [0, 1, 2, 3]
        ]:
            feature_cols = []
            for idx in feature_combo:
                feature_cols.extend(self.feature_combinations[idx])
                ...
                param_combinations = itertools.product(
                    ...
                    self.optimization_params['rsi_periods'],
                    self.optimization_params['adx_periods'],
                    self.optimization_params['cci_periods'],
                    self.optimization_params['wt_periods']
                )
                ...
                for params_tuple in param_combinations:
                    params = {
                        ...
                        'rsi_period': params_tuple[3],
                        'adx_period': params_tuple[4],
                        'cci_period': params_tuple[5],
                        'wt_period': params_tuple[6]
                    }
    ...
    def train_optimized_model(self, df: pd.DataFrame, optimization_result: Dict) -> Tuple[LorentzianANN, Dict]:
        ...
        features_df = self.prepare_features_with_params(df, optimization_result['best_params'])
