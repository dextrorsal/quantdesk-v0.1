...
-- Chat messages (updated to support channels)
CREATE TABLE chat_messages (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    channel_id UUID NOT NULL REFERENCES chat_channels(id) ON DELETE CASCADE,
    author_pubkey TEXT NOT NULL,
    message TEXT NOT NULL,
    mentions JSONB DEFAULT '[]', -- Array of mentioned wallet pubkeys
    is_pinned BOOLEAN DEFAULT false,
    is_system BOOLEAN DEFAULT false, -- For system announcements
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE INDEX idx_chat_messages_author_pubkey ON chat_messages(author_pubkey);
...
CREATE POLICY "Users can create channels" ON chat_channels
    FOR INSERT WITH CHECK (auth.jwt() ->> 'wallet_address' = (
        SELECT wallet_address FROM users WHERE id = created_by
    ));

CREATE POLICY "Channel creators can update their channels" ON chat_channels
    FOR UPDATE USING (auth.jwt() ->> 'wallet_address' = (
        SELECT wallet_address FROM users WHERE id = created_by
    ));
...
CREATE POLICY "Users can send messages" ON chat_messages
    FOR INSERT WITH CHECK (auth.jwt() ->> 'wallet_address' = author_pubkey);

CREATE POLICY "Users can edit their own messages" ON chat_messages
    FOR UPDATE USING (auth.jwt() ->> 'wallet_address' = author_pubkey AND deleted_at IS NULL);

CREATE POLICY "Users can delete their own messages" ON chat_messages
    FOR UPDATE USING (auth.jwt() ->> 'wallet_address' = author_pubkey);
